...
function IncomeItem({ income, onDelete, onUpdateSuccess, indicatorColor }) { // Added indicatorColor here
    ...
    const categoryIcon = () => {
        switch (category) {
            ...
            case 'other':
                return piggy;
            default:
                return ''; // Return empty string or a default icon if category doesn't match
        }
    };
    ...
    const handleSaveClick = async () => {
        try {
            ...
            if (!editedIncome.title || !editedIncome.amount || !editedIncome.category || !editedIncome.start_date) {
                alert('Please fill in all required fields.');
                return;
            }
            if (editedIncome.amount <= 0) {
            ...
            <div className="content">
                {isEditing ? (
                    ...
                    <div className="edit-form">
                        ...
                        <DatePicker
                            name="start_date"
                            value={editedIncome.start_date}
                            onChange={(date) => handleDateChange(date, "start_date")}
                        />
                        <DatePicker
                           type="end_date"
                            name="end_date"
                            value={editedIncome.end_date}
